...
export function BookingSummary({ event, representative, day, slotTime, disabled, pending, error, onConfirm }: { event: AgendaEvent; representative: PublicRepresentative; day: string; slotTime: string | null; disabled: boolean; pending: boolean; error: string | null; onConfirm: () => void }) {
  return (
    <aside className="rounded-2xl bg-blue-50 p-6 lg:sticky lg:top-5">
      ...
      <button type="button" disabled className="mt-3 flex w-full items-center justify-center gap-2 rounded-full border-2 border-green-500 px-5 py-3 text-sm font-bold text-green-700 opacity-55"><AgendaIcon name="whatsapp" className="h-5 w-5"/>Agendar por WhatsApp</button>
