...
export function SlotPicker({ days, slots, selectedDay, selectedTime, loading, onDay, onTime }: { days: string[]; slots: AgendaSlot[]; selectedDay: string; selectedTime: string | null; loading: boolean; onDay: (day: string) => void; onTime: (time: string) => void }) {
  ...
  function selectByKeyboard(event: React.KeyboardEvent<HTMLButtonElement>, index: number) {
    ...
    const target = event.key === "Home" ? 0 : event.key === "End" ? days.length - 1 : direction ? (index + direction + days.length) % days.length : null;
    if (target === null) return;
    event.preventDefault(); onDay(days[target]);
    requestAnimationFrame(() => document.getElementById(`agenda-day-${target}`)?.focus());
  }
  return <div className="mt-7"><div role="tablist" aria-label="Días disponibles" className="grid grid-cols-3 gap-3">{days.map((day, index) => <button id={`agenda-day-${index}`} key={day} type="button" role="tab" aria-selected={day === selectedDay} aria-controls="agenda-slots" tabIndex={day === selectedDay ? 0 : -1} onKeyDown={(event) => selectByKeyboard(event, index)} onClick={() => onDay(day)} className={`rounded-xl px-3 py-4 text-sm font-bold capitalize ${day === selectedDay ? "bg-navy text-white" : "bg-blue-50 text-navy"}`}>{dayLabel(day)}</button>)}</div><div id="agenda-slots" role="tabpanel" className="mt-5 grid grid-cols-3 gap-3 sm:grid-cols-6">{loading ? Array.from({ length: 6 }, (_, index) => <span key={index} className="h-12 animate-pulse rounded-xl bg-blue-50"/> ) : daySlots.map((slot) => <button key={`${slot.day}-${slot.slotTime}`} type="button" disabled={slot.taken} onClick={() => onTime(slot.slotTime)} className={`rounded-xl px-2 py-3 text-sm font-bold ${slot.taken ? "cursor-not-allowed bg-gray-50 text-ink/35 line-through" : selectedTime === slot.slotTime ? "bg-blue text-navy ring-2 ring-navy" : "bg-blue-50 text-navy hover:bg-blue-mid/50"}`}>{slot.slotTime.slice(0, 5)}{slot.taken && <span className="sr-only"> ocupado</span>}</button>)}</div></div>;
}
