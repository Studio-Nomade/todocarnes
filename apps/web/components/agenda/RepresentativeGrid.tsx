  ...
  return (
    <div className="mt-7 flex flex-wrap justify-center gap-4">
      {representatives.map((rep) => {
        ...
        return <article key={rep.id} className={`group/representative relative flex w-full flex-col rounded-2xl border bg-white p-4 shadow-sm transition sm:w-[calc(50%-8px)] lg:w-[calc(25%-12px)] ${selected ? "border-blue ring-2 ring-blue/30" : "border-navy/10 hover:border-blue-mid"}`}>
          ...
          {rep.photoUrl ? <div className="relative aspect-[4/3] overflow-hidden rounded-xl bg-blue-50"><Image src={rep.photoUrl} alt={`Retrato de ${rep.name}`} fill className="object-cover transition-[filter] duration-300 motion-reduce:transition-none [@media(hover:hover)_and_(pointer:fine)]:grayscale group-hover/representative:grayscale-0 group-focus-within/representative:grayscale-0" sizes="(min-width: 1024px) 25vw, 50vw"/></div> : <div className="grid aspect-[4/3] place-items-center rounded-xl bg-gradient-to-br from-blue-50 to-blue-mid/40"><span className="grid h-20 w-20 place-items-center rounded-full bg-white font-display text-3xl text-navy shadow-sm">{rep.name.split(" ").map((part) => part[0]).slice(0, 2).join("")}</span></div>}
