...
function emptyForm(eventId: string): CourtesyRequestInput {
  return { eventId, name: "", company: "", cargo: "", email: "", phone: "", area: "", website: "" };
}
    ...
    <section id="entrada-cortesia" className="scroll-mt-24 bg-blue-50 px-5 py-14 sm:px-8 lg:py-20">
      <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[0.75fr_1.25fr]">
        ...
        <form onSubmit={submit} className="grid gap-5 sm:grid-cols-2">
          <label className="text-sm font-semibold text-navy">Nombre *<input required autoComplete="name" value={form.name} onChange={(event) => update("name", event.target.value)} className={fieldClass}/></label>
          ...
          <label className="text-sm font-semibold text-navy">Área de interés *<select required value={form.area} onChange={(event) => update("area", event.target.value)} className={fieldClass}><option value="">Selecciona un área</option>{LANDING_AREA_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
          ...
          <p aria-live="polite" className="text-sm font-semibold text-navy sm:col-span-2">
            ...
            {status === "error" && "No pudimos registrar la solicitud. Revisa los datos e inténtalo nuevamente."}
          </p>
        </form>
      </div>
    </section>
